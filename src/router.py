...
from .. import app, processes, scene, applied_actions, tick_number
...
async def tick():
    ...
    actions.extend([Entity.deserialize(pending_action) for pending_action in scene.pending_actions])
...
async def populate():
    ...
    for (x,y) in itertools.product(range(100), range(10)):
        scene.add_entity(Object(id=id, name="Grass", position=Vector(x,y), walkable=True))
        id += 1
        if opensimplex.noise2(x,y) > .3:
            scene.add_entity(Object(id=id, name="Tree", position=Vector(x,y), walkable=False))
            id += 1
